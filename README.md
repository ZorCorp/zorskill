...
| `bms2` | Query the Master Concept BMS operational database (orders, invoices, gross profit, purchase orders / vendor spend, suppliers, customers, products, Google GCP/GMP cost & recovery, exchange rates, sales attribution, org chart) from its read-only BigQuery mirror `mc-bms2.bms2_mysql8_bq` — a near-real-time copy of the BMS2 MySQL 8 DB (~1.7 GB). Read-only, no VPN. | [ZorCorp/bms2](https://github.com/ZorCorp/bms2) |
